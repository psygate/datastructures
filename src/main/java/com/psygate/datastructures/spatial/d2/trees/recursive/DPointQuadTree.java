...
public class DPointQuadTree<K extends IDPoint, V> implements IDPointQuadTree<K, V> {
    ...
    public DPointQuadTree(IDPointQuadTree<K, V> orig) {
        ...
        root = new QuadNode<>(new DBoundingBox(Objects.requireNonNull(orig.getBounds())), this);
    ...
    public DPointQuadTree(int maxNodeSize, IDBoundingBox bounds) {
        ...
        root = new QuadNode<>(new DBoundingBox(Objects.requireNonNull(bounds)), this);
    }
    ...
    public boolean checkIntegrity() {
        assert root != null;
        Set<QuadNode<K, V>> visited = new HashSet<>();
        Queue<QuadNode<K, V>> stack = new LinkedList<>();
        stack.add(root);
    ...
    private final class SelectiveTreeNodeSpliterator implements Spliterator<QuadNode<K, V>> {

        private final LinkedList<QuadNode<K, V>> stack = new LinkedList<>();
        private final long thiscount = modcnt.get();
        private final Predicate<IDBoundingBox> selector;

        SelectiveTreeNodeSpliterator(QuadNode<K, V> node, Predicate<IDBoundingBox> selector) {
            stack.add(node);
    ...
    private final class TreeNodeSpliterator implements Spliterator<QuadNode<K, V>> {

        private final LinkedList<QuadNode<K, V>> stack = new LinkedList<>();
        private final long thiscount = modcnt.get();

        TreeNodeSpliterator(QuadNode<K, V> node) {
            stack.add(node);
        }
    ...
    private final class NodeIterator implements Iterator<QuadNode<K, V>> {

        private final Deque<QuadNode<K, V>> stack = new LinkedList<>();
        private final Predicate<IDBoundingBox> selector;
        private final long thiscount = modcnt.get();
        private final List<QuadNode> visited = new LinkedList<>();

        private NodeIterator(QuadNode<K, V> node, Predicate<IDBoundingBox> selector) {
...
//        }
    ...
    private final class KeyIterator implements Iterator<K> {
        ...
        KeyIterator(QuadNode<K, V> node, Predicate<IDBoundingBox> selector) {
    ...
    private final class ValueIterator implements Iterator<V> {
        ...
        ValueIterator(QuadNode<K, V> node, Predicate<IDBoundingBox> selector) {
            it = new EntryIterator(node, selector);
        }
    ...
    private final class EntryIterator implements Iterator<Map.Entry<K, V>> {
        ...
        EntryIterator(QuadNode<K, V> node, Predicate<IDBoundingBox> selector) {
